...
2. 開発サーバー
   ```bash
   pnpm dev:server    # @repojiten/backend-entry (http://localhost:8787)
   pnpm dev:client    # @repojiten/frontend-app  (http://localhost:5173)
   # または
   pnpm dev:all
   ```
...
pnpm test:client   # @repojiten/frontend-app
pnpm test:server   # @repojiten/backend-http
pnpm test:ui       # @repojiten/frontend-ui
